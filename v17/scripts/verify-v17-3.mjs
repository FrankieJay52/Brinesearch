...
const [dashboard, feed, router, polish, finalFixes, roadLiveFixes, roadLiveStyles, packageJson, partManifest, styleManifest, sw, liveDirections, directionPolish, directionLandmarkCleanup, directionRoadIntel, directionTargetRoadFix, directionCompoundSafety, directionRoadRebuild, directionRoadIntelStyles, publicDataReview, publicDataReviewStyles, publicPadCard, publicPadCardStyles, driverRouteReference, driverRouteReferenceAudit, driverRouteReferenceStyles, verificationCleanup, verificationCleanupStyles, wellListPolish, wellListPolishStyles] = await Promise.all([
  ...
  read(path.join(v17Root,'src/parts/22f-direction-road-rebuild.js')),
...
for (const style of ['34-public-data-review.css','35-public-data-pad-card.css','36-driver-route-reference.css','37-verification-cleanup.css','38-direction-road-intelligence.css','39-well-list-polish.css']) requireText(styleManifest,style,'Style manifest');
for (const part of ['09a-public-data-review.js','11b-public-data-pad-card.js','11c-driver-route-reference.js','11d-verification-cleanup.js','11e-route-reference-audit-fixes.js','11f-well-list-polish.js','22b-direction-landmark-cleanup.js','22c-direction-road-intelligence.js','22d-direction-target-road-fix.js','22e-direction-compound-step-safety.js','22f-direction-road-rebuild.js']) requireText(partManifest,part,'Part manifest');
...
requireText(sw,'brinesearch-v17-3-12-road-direction-rebuild','Service-worker cache version');
...
requireText(directionRoadRebuild,'directionGenericRouteLabelV17312','Canonical route-family renderer');
requireText(directionRoadRebuild,'directionSplitDriverStepV17312','Compound driver-step splitter');
requireText(directionRoadRebuild,'directionRoadIntelRoadRebuiltV17312','Canonical road-intelligence display');
requireText(directionRoadRebuild,'directionSequenceSummaryRebuiltV17312','Canonical route summary');
...
for (const token of ['Current Pad Snapshot','Official pad name','Official location','Pad permit','Saved API check','Official match basis','await publicDataPadCardBaseV1736(id)','official_well_records','saved driver/navigation point is unchanged']) requireText(publicPadCard,token,'Driver public-data snapshot');
...
for (const token of ['brinesearch_driver_route_reference','Known Approach Road','ROAD TYPE','DISTANCE MEANS','is_stale','not proven a usable truck connection']) requireText(driverRouteReference,token,'Driver route-reference');
...
console.log('Verified BrineSearch V17.3.12 product layer: rebuilt driver directions, canonical highway families, dual road-name support, pad-specific mileage safety, synchronized wells, and current version markers.');
