...
requireText(sw,'brinesearch-v17-3-5-driver-route-reference','Service-worker cache version');
...
requireText(driverRouteReference,'Approach Reference','Driver route-reference card');
requireText(driverRouteReference,'TO PAD / ACCESS','Driver route-reference distance label');
requireText(driverRouteReference,'not proven a usable truck connection','Research-only safety boundary');
requireText(driverRouteReferenceStyles,'.driver-route-reference-card','Driver route-reference visual layer');
...
console.log('Verified BrineSearch V17.3.5 product layer: driver approach reference, confirmed official-data pad snapshot, Owner Public Data Review, authoritative live Clear Directions, Road Manager, installed-app updates, and current version markers.');