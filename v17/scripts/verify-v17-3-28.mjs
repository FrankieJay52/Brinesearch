...
const [pkgText, partsText, stylesText, interactive, canonical, stepEditor, strictLookup, stepCss, sw, audit] = await Promise.all([
  ...
  read(path.join(v17Root, 'src/parts/21h-road-manager-step-tap-lookup-bound-v17328.js')),
...
const strictIndex = parts.parts.indexOf('21h-road-manager-step-tap-lookup-bound-v17328.js');
const directions = parts.parts.indexOf('22m-direction-source-priority-and-coalescer.js');
if (interactiveIndex < 0 || canonicalIndex <= interactiveIndex || stepIndex <= canonicalIndex || strictIndex <= stepIndex || directions <= strictIndex) throw new Error('V17.3.28 route-editor load order is incorrect.');
...
for (const token of ['routeInteractiveLookupRoadsStrictBoundV17328', 'overpass.private.coffee', 'ROUTE_STEP_EXTERNAL_TIMEOUT_MS_V17328']) if (!strictLookup.includes(token)) throw new Error(`V17.3.28 strict tap lookup missing ${token}.`);
...
console.log('Verified BrineSearch V17.3.28: selecting a route step snaps the map to that road, the selected step is highlighted separately, the Owner can pan and tap a correct road to move/replace it, insert roads before/after, remove roads, edit turn/mileage in full screen, and the map tap lookup stays locally-first and strictly time-bounded before the existing Save route & update directions publisher.');
