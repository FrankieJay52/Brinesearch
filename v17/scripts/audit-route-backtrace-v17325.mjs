...
const [source, refinements] = await Promise.all([
  fs.readFile(path.join(scriptDir, '../src/parts/21b-road-manager-backtrace-route-v17325.js'), 'utf8'),
  fs.readFile(path.join(scriptDir, '../src/parts/21c-road-manager-backtrace-refinements-v17325.js'), 'utf8')
]);
...
vm.runInContext(refinements, context, { filename: '21c-road-manager-backtrace-refinements-v17325.js' });

for (const fn of ['routeBacktraceRoadTokensV17325', 'routeBacktraceBuildTraceV17325', 'routeBacktraceTokenPartsV17325']) {
...
if (tokens[0].kind !== 'access' || tokens[4].kind !== 'exit' || tokens[1].kind !== 'road') throw new Error('Backward token classification failed.');
if (context.routeBacktraceSimilarityV17325('Muscrat Rd', 'Muskrat Rd') < 0.8) throw new Error('Minor saved-road spelling differences are not fuzzy matched.');
if (context.routeBacktraceTokenPartsV17325('CR-102 / Mt Olivett Rd').length !== 2) throw new Error('Dual road identity was not split for map matching.');
...
if (matched.length !== 4) throw new Error(`Expected four matched public roads, found ${matched.length}: ${trace.map(row => `${row.token.label}:${row.matched}`).join(', ')}`);
if (!matched[0].traceLine?.length || !matched[1].traceLine?.length || !matched[2].traceLine?.length) throw new Error('Connected backward roads did not produce highlighted trace geometry.');

const crossing = context.routeBacktraceClosestPairV17325([[-0.02, 0], [-0.02, -0.02]], [[-0.04, -0.02], [0, -0.02]]);
if (crossing.distance > 0.01) throw new Error(`Segment-aware road intersection failed: ${crossing.distance} mi.`);
...
if (!missing.some(row => row.token.label === 'Mystery Access Rd' && !row.matched)) throw new Error('Unmatched road was not preserved as an untraced no-guess gap.');
...
for (const token of ['Structured_Road_Sequence', 'routeBacktraceOverpassDualPatternV17325', 'routeBacktraceClosestOnSegmentV17325']) {
  if (!refinements.includes(token)) throw new Error(`Backtrace refinements missing ${token}.`);
}
...
console.log(JSON.stringify({
  ...
  segmentAwareIntersections: 'pass',
  dualRoadIdentity: 'pass',
  noGuessGap: 'pass',
