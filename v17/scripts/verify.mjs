...
for (const requiredPublicDataToken of ['public-pad-data-card', 'Current Pad Snapshot', 'Official pad name', 'Official location', 'Pad permit', 'Saved API check', 'Official match basis', 'Public Data / Well Details', 'official_well_records', 'saved driver/navigation point is unchanged']) {
...
console.log(`Verified V${parts.version}: ${parts.parts.length} JS parts, ${styles.styles.length} CSS parts, richer driver route intelligence, richer official pad context, authoritative live Clear Directions, clean index, and live-updating service worker. Build hashes: ${appSha256.slice(0,12)} / ${cssSha256.slice(0,12)}.`);
