    ...
    const publicDataPadCardBaseV1734 = renderPad;

    function publicDataPadCardDateV1734(value) {
    ...
    function publicDataPadCardSummaryV1734(p) {
    ...
    function publicDataPadCardHtmlV1734(p) {
      const data = publicDataPadCardSummaryV1734(p);
      if (!data) return "";
      const wellCount = data.wells.length;
      const sourceLabel = data.source?.agency || padVerificationSource(p);
      const latestLabel = publicDataPadCardDateV1734(data.latest) || "Not listed";
      return `<section class="public-pad-data-card" id="publicPadDataCard">
        ...
        <p>Official pad centers and wellheads are reference data only. Your saved driver/navigation point is unchanged.</p>
      </section>`;
    }

    renderPad = function publicDataPadCardRenderV1734(id) {
      const result = publicDataPadCardBaseV1734(id);
      const p = padById(id);
      if (!p) return result;
      const html = publicDataPadCardHtmlV1734(p);
