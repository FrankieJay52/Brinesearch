    ...
    function renderPad(id) {
      ...
      const googleRouteChunks = googleMapsRouteChunks(p);
      const googleRouteReady = googleRouteChunks.length > 0;
        ...
        <div class="field-pad-page">
          ...
          <section class="field-pad-hero">
            ...
            <div class="field-main-actions">
              ${googleRouteReady ? googleRouteChunks.map((chunk, index) => `<a class="field-action ${index === 0 ? "primary " : ""}field-action-visual field-action-directions" data-google-route-chunk="${index + 1}" data-google-route-count="${googleRouteChunks.length}" href="${esc(chunk.url)}" target="_blank" rel="noopener"><span class="field-action-emblem"><span class="fm-icon fm-directions-large"></span></span><span class="field-action-copy"><strong>${googleRouteChunks.length === 1 ? "Google Maps route" : `Google route ${index + 1} of ${googleRouteChunks.length}`}</strong><small>${index === 0 ? "Authoritative GPS-waypoint navigation" : "Starts at the previous chunk destination"}</small></span></a>`).join("") : (gmap ? `<a class="field-action primary field-action-visual field-action-directions" href="${esc(gmap)}" target="_blank" rel="noopener"><span class="field-action-emblem"><span class="fm-icon fm-directions-large"></span></span><span class="field-action-copy"><strong>Google Maps GPS</strong><small>Exact pad pin only — route not yet verified</small></span></a>` : `<span class="field-action is-unavailable"><span class="fm-icon fm-warning"></span> No GPS saved — use written directions below</span>`)}
