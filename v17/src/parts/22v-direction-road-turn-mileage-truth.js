    ...
    function directionTruthTransitionSplitV17320(entry) {
    ...
    function directionTruthStayThenTurnSplitV17320(entry, previousRoad) {
      const leading = directionTruthLeadingV17320(entry?.instruction || "");
      const work = leading.work;
      if (!work) return null;
      ...
      let secondRaw = directionTruthNormalizeTurnTailV17320(match[3]);
      if (!secondRaw) return null;
      const secondTarget = secondRaw.match(/^Turn\s+(left|right)\b([\s\S]*?)\bon\s+(.+)$/i);
      const secondNotes = [...(entry?.notes || [])];
      if (secondTarget) {
        const context = directionTruthCleanV17320(secondTarget[2]);
        if (context) {
          const note = directionTruthNoteFromBeforeTargetV17320(context);
          if (note) secondNotes.push(note);
        }
        secondRaw = `Turn ${secondTarget[1].toLowerCase()} on ${directionTruthCleanV17320(secondTarget[3])}`;
      }
    ...
    function directionTruthDistanceBeforeTurnSplitV17320(entry, previousRoad, p) {
      ...
      for (const fact of facts) {
        ...
        if (!directionTruthRoadV17320(firstRaw, p)) firstInstruction = `Continue on ${firstRoad} for ${fact.raw}`;
    ...
    function directionTruthThenTurnSplitV17320(entry) {
      const leading = directionTruthLeadingV17320(entry?.instruction || "");
      const work = leading.work;
      if (!work) return null;
      ...
      if (!/\b(?:turn|stay|keep|veer|bear|slight|head|take|follow|continue|travel|proceed|go)\b/i.test(first)) return null;
      return [
    ...
    function directionTruthSplitOneV17320(entry, previousRoad, p) {
      if (entry?.sourceTruthSplitV17320) return [entry];
      return directionTruthTransitionSplitV17320(entry)
        || directionTruthStayThenTurnSplitV17320(entry, previousRoad)
        || directionTruthDistanceBeforeTurnSplitV17320(entry, previousRoad, p)
        || directionTruthThenTurnSplitV17320(entry)
        || [entry];
    }
    ...
    function directionTruthEntriesV17320(entries, p) {
      ...
      for (const original of entries || []) {
        const parts = directionTruthSplitOneV17320(original, previousRoad, p);
        for (const finalEntry of parts) {
          out.push(finalEntry);
