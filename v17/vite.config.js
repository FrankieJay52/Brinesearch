...
const releaseHtml = {
  ...
  transformIndexHtml(html) {
    return html
      ...
      .replace(/<meta\s+name=["']brinesearch-build["']\s+content=["'][^"']*["']\s*\/?>(?=)/i, '<meta name="brinesearch-build" content="17.3.1">')
      .replace(/(<link\s+rel=["']stylesheet["']\s+href=["']\/styles\/app\.css[^"']*["']\s*>)/i, '$1\n<link rel="stylesheet" href="/styles/field-mark-icons.css?v=17.3.1">')
      ...
      .replace(/ · V 16\.21<\/div>/g, ' · V 17.3.1</div>')
      .replace(/<script\s+src=["']\/v16-25-hotfix\.js[^"']*["']><\/script>/i, '<script src="/app/front-sign-structured.js?v=17.3.1"></script>\n<script src="/app/front-sign-hidden.js?v=17.3.1"></script>')
      .replaceAll('?v=17.0.0', '?v=17.3.1')
      .replaceAll('?v=17.3.0', '?v=17.3.1');
  }
};
