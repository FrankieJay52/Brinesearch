...
js_path = Path("v17/src/parts/18-road-manager-route-prep.js")
css_path = Path("v17/src/styles/29-road-manager-route-prep.css")
text = js_path.read_text()

# Safe to run repeatedly. The second run only removes this helper and exits.
if "function roadManagerFamilyGroups" in text:
    helper = Path(".github/scripts/patch-road-manager-route-families.py")
    workflow = Path(".github/workflows/patch-road-manager-route-families.yml")
    if workflow.exists():
        workflow.unlink()
    if helper.exists():
        helper.unlink()
...
text = text.replace(
    'Master roads, 833-pad Route Prep, mileage readiness and no-guess review',
    'Grouped road families, live Route Prep, mileage readiness and strict no-guess review'
)

family_block = r'''    let roadManagerFamilies=[];
    ...
    function routePrepSummaryCard'''
text, count = re.subn(
    r"    function renderMasterRoadTab\(\)\{.*?\n    \}\n\n    function routePrepSummaryCard",
    family_block,
    text,
    count=1,
    flags=re.S,
)
assert count == 1, "renderMasterRoadTab block was not replaced"

text = text.replace(
    'const order=["ready_for_road_matching","needs_state_route_candidate_review","needs_highway_anchor",',
    'const order=["ready_for_road_matching","needs_highway_anchor",'
)
text = text.replace(
...
text = text.replace("Change a filter or refresh the 833-pad analysis.", "Change a filter or refresh the live pad-route analysis.")
text = text.replace("Loading the 833-pad preparation summary…", "Loading the live pad-route preparation summary…")
text = text.replace(
    "State-route candidates remain review-only. Local roads never receive an educated guess.",
    "A bare Route number stays ambiguous until evidence proves the route class. Local roads never receive a guess."
)
text = text.replace(
...
text = text.replace(
    '${candidate&&step.owner_decision==="pending"?`<button',
    '${candidate&&source.state_route_supported===true&&step.owner_decision==="pending"?`<button'
)
text = text.replace(
    "Only saved road names and exact Road Manager/official matches are used. A state-route candidate is review-only and cannot publish.",
    "Only saved road names and exact Road Manager or official matches are used. A bare Route number stays ambiguous, and any supported research candidate remains non-publishable."
)
text = text.replace(
    "One shared road database plus a strict preparation queue for all 833 pads that have GPS and a saved road sequence.",
    "One grouped road database plus a strict preparation queue for every pad that has GPS and a saved road sequence."
)

assert "833-pad" not in text
assert "all 833 pads" not in text
assert "function roadManagerFamilyGroups" in text
assert "ambiguous_generic_route_reference" in text
js_path.write_text(text)

css = css_path.read_text()
addition = r'''

/* Grouped highway/local road families */
.road-family-row .road-row-meta{line-height:1.4}.road-status.conflict{color:#ffd087;border-color:color-mix(in srgb,#ffbd66 48%,var(--line));background:color-mix(in srgb,#ffbd66 8%,transparent)}.road-status.candidate{color:#d7b8ff;border-color:color-mix(in srgb,#c69cff 48%,var(--line));background:color-mix(in srgb,#c69cff 8%,transparent)}
.road-family-editor{margin-top:12px;border:1px solid var(--line);border-radius:20px;background:var(--panel);padding:15px;box-shadow:0 12px 30px rgba(0,0,0,.13)}.road-family-editor-head{display:flex;align-items:flex-start;justify-content:space-between;gap:12px}.road-family-editor-head h2{margin:0;font-size:1.15rem}.road-family-editor-head p{margin:5px 0 0;color:var(--muted);font-size:.72rem}.road-family-members{display:grid;gap:8px;margin-top:12px}.road-family-member{display:flex;align-items:center;justify-content:space-between;gap:12px;padding:11px;border:1px solid var(--line);border-radius:14px;background:var(--panel-2)}.road-family-member>div{min-width:0}.road-family-member strong{display:block;font-size:.84rem}.road-family-member small,.road-family-member span{display:block;margin-top:4px;color:var(--muted);font-size:.62rem;line-height:1.35;overflow-wrap:anywhere}.road-family-conflict,.road-family-note{margin-top:12px;padding:10px 11px;border-radius:13px;font-size:.7rem;line-height:1.45}.road-family-conflict{border:1px solid color-mix(in srgb,#ffbd66 38%,var(--line));background:color-mix(in srgb,#ffbd66 7%,transparent)}.road-family-conflict p{margin:4px 0 0;color:var(--muted)}.road-family-note{border:1px solid color-mix(in srgb,var(--accent) 30%,var(--line));background:color-mix(in srgb,var(--accent) 6%,transparent);color:var(--muted)}
.route-prep-ambiguous-note{margin-top:8px;padding:8px 9px;border-radius:10px;font-size:.65rem;line-height:1.4;background:color-mix(in srgb,#ffbd66 7%,transparent);border:1px solid color-mix(in srgb,#ffbd66 28%,var(--line))}
@media(max-width:600px){.road-family-editor-head,.road-family-member{display:grid}.road-family-member .btn{width:100%}}
'''
if "/* Grouped highway/local road families */" not in css:
    css += addition
...
if workflow.exists():
    workflow.unlink()
Path(".github/scripts/patch-road-manager-route-families.py").unlink()
