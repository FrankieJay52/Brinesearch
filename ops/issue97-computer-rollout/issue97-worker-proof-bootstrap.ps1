...
$success = $false
$failureCode = 'bootstrap_initialization_failed'
...
try {
  ...
  if ([string]$claim.manifest_sha256 -ne
    ...
    throw 'production launch claim does not bind the manifest bytes'
  }
  ...
  if ([string]$manifest.private_log_root -ne $logRoot -or
    ...
    throw 'production launch claim/manifest scope mismatch'
  }
  ...
  foreach ($pair in @(
    ...
    @($workerPath, 'ops/issue97-computer-rollout/issue97-worker-proof-worker.ps1')
  )) {
    ...
    if ($artifact.Count -ne 1 -or $null -eq $claimHash -or
      ...
      throw 'production launch claim does not bind bootstrap artifacts'
    }
  }
  ...
  if (-not (Test-Issue97SamePath -Left ([string]$claim.repo_root) -Right $repoRoot) -or
    ...
    throw 'production launch claim runtime path/hash mismatch'
  }
  ...
  foreach ($key in $artifactHashes.Keys) {
    if ([string]$claim.artifact_hashes.$key -ne [string]$artifactHashes[$key]) {
      throw 'production launch claim artifact hash map mismatch'
    }
  }
  ...
  $attempt = [ordered]@{
    ...
    bootstrap_script = $bootstrapPath
    worker_script = $workerPath
    sql_path = (Join-Path $repoRoot 'ops\issue97-computer-rollout\sql\35-worker-proof-read-only.sql')
    psql_path = $psqlPath
    psql_sha256 = [string]$manifest.psql.sha256
  }
  Write-Issue97JsonAtomicNoClobber -LiteralPath $attemptPath -Value $attempt

  $stage = 'spawn_detached_worker'
  ...
  if ([int]$created.ReturnValue -ne 0 -or [int]$created.ProcessId -le 0) {
    throw 'WMI failed to create the fixed production worker'
  }
  ...
  $failureCode = "bootstrap_fail_stop_$stage"
} finally {
  ...
  if ($null -ne $claim) {
    ...
    try { Write-Issue97JsonAtomicNoClobber -LiteralPath $bootstrapFinalPath -Value $finalReceipt } catch { exit 72 }
  }
  if ($success) { exit 0 }
  exit 71
}
