  ...
  if (select details->>'mapping_snapshot_digest' from public.brinesearch_road_graph_builds
     ...
     or exists(select 1 from public.brinesearch_road_graph_builds build
       where build.id=any(array[
         ...
         build.details->>'mapping_snapshot_upgrade_reason'<>'harrison_har_to_has_wrong_geometry_repair'
         or build.details->'mapping_snapshot_upgrade_proof'->>'topology_changed'<>'false'
         or build.details->'mapping_snapshot_upgrade_proof'->>'source_run_vector_unchanged'<>'true'
       ))
     or not exists(select 1 from public.brinesearch_road_graph_builds build
       where build.id='5ee5f97b-447f-41d3-946a-68a8b28d8367'
         and build.graph_digest='e9f4ed56bb2b1308b8b9913a751c78f5'
...
select pg_catalog.jsonb_build_object(
  ...
  'ohio_source_scopes',(select pg_catalog.md5(coalesce(pg_catalog.string_agg(
    pg_catalog.to_jsonb(row_value)::text,'|' order by row_value.dataset_id,row_value.state_code,row_value.county_code),''))
    from public.brinesearch_road_source_dataset_counties row_value where row_value.state_code='OH'),
