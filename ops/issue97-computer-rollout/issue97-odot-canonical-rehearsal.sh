...
require_connection_profile() {
  ...
  export PGAPPNAME="brinesearch-issue97-odot-canonical-rehearsal"
}
...
select pg_catalog.jsonb_build_object(
  ...
  'generations',(select coalesce(pg_catalog.jsonb_agg(pg_catalog.to_jsonb(g)
    order by g.generation_key),'[]'::jsonb)
    from private_verification.brinesearch_issue97_graph_release_generations g),
  'qualifications',(select coalesce(pg_catalog.jsonb_agg(pg_catalog.to_jsonb(q)
    order by q.build_id,q.generation_key),'[]'::jsonb)
    from private_verification.brinesearch_issue97_graph_release_qualifications q),
  'qualification_pk',(select pg_catalog.pg_get_constraintdef(c.oid)
    from pg_catalog.pg_constraint c
    where c.conrelid='private_verification.brinesearch_issue97_graph_release_qualifications'::pg_catalog.regclass
      and c.contype='p'),
...
begin
  if public.brinesearch_issue97_cutover_active()
    ...
    raise exception 'Issue #97 ODOT hotfix rehearsal requires cutover OFF and staging zero';
  end if;
  if exists(select 1 from pg_catalog.pg_stat_activity activity
      where activity.pid<>pg_catalog.pg_backend_pid()
        ...
        and (activity.query ilike '%brinesearch_issue97_rebuild_county_graph%'
          or activity.application_name='brinesearch-issue97-odot-canonical-rehearsal'
          or activity.application_name='brinesearch-issue97-odot-canonical-install')) then
    raise exception 'Issue #97 ODOT hotfix rehearsal refuses a concurrent builder/installer';
  end if;
  if (select count(*) from supabase_migrations.schema_migrations
      where version in (${base_versions}))<>22
     or exists(select 1 from supabase_migrations.schema_migrations
       where version='${migration_version}') then
    raise exception 'Issue #97 ODOT hotfix rehearsal requires base 22/22 and hotfix 0/1';
  end if;
  if pg_catalog.md5(pg_catalog.pg_get_functiondef(
    ...
    raise exception 'Issue #97 ODOT hotfix rehearsal predecessor builder/generation changed';
  end if;
  if (select count(*) from private_verification.brinesearch_issue97_saved_road_reconciliation_runs)<>0
    ...
    raise exception 'Issue #97 ODOT hotfix rehearsal requires reconciliation/publication zero';
  end if;
  ...
  if v_required<>38 or v_current<>38 then
    raise exception 'Issue #97 ODOT hotfix rehearsal requires Ohio source scopes 38/38 current';
  end if;
  if (select count(*) from public.brinesearch_road_graph_builds b
    ...
    raise exception 'Issue #97 ODOT hotfix rehearsal canary checkpoint changed';
  end if;
end
\$gate\$;
SQL
...
main() {
  ...
  [[ $# -eq 0 ]] || die "ODOT canonical rollback rehearsal accepts no arguments"
  require_repo_checkpoint
  require_connection_profile
  verify_file
  run_psql --command="${preflight_sql}" >/dev/null
  before="$(run_psql --tuples-only --no-align --quiet --command="${snapshot_sql}")"
  [[ -n "${before}" ]] || die "failed to capture hotfix before snapshot"

  umask 077
  mkdir -p "${log_dir}"
  sql_file="$(mktemp "${TMPDIR:-/tmp}/issue97-odot-canonical-rehearsal.XXXXXX.sql")"
  log_file="${log_dir}/$(date -u +%Y%m%dT%H%M%SZ)-odot-canonical-rehearsal.log"
  trap 'if [[ -n "${sql_file:-}" ]]; then rm -f -- "${sql_file}"; fi' EXIT
  {
    ...
    printf "  if pg_catalog.md5(pg_catalog.pg_get_functiondef('public.brinesearch_issue97_rebuild_county_graph(text,text)'::pg_catalog.regprocedure))<>'06705f5b35a6d37151bb2c0dc5ade9bd' then raise exception 'Issue #97 hotfix rehearsal builder mismatch'; end if;\n"
    printf "  if private_verification.brinesearch_issue97_active_graph_release_generation()<>'issue97-release-20260815-r2' then raise exception 'Issue #97 hotfix rehearsal generation mismatch'; end if;\n"
    printf "  if (select count(*) from supabase_migrations.schema_migrations where version='${migration_version}')<>1 then raise exception 'Issue #97 hotfix rehearsal history mismatch'; end if;\n"
    printf "  if not private_verification.brinesearch_issue97_graph_build_release_current('44d1d8dd-9f1f-4455-bd14-739f933c78ee'::uuid) then raise exception 'Issue #97 hotfix rehearsal OHI compatibility failed'; end if;\n"
    printf 'end $verify$;\n'
    printf '\\echo Issue #97 exact-one ODOT canonical hotfix compiled and verified inside transaction\n'
    printf 'rollback;\n'
  } > "${sql_file}"

  printf 'ODOT canonical rehearsal log: %s\n' "${log_file}"
  set +e
  ...
  if [[ "${rc}" -ne 0 ]]; then
    ...
    if [[ -n "${after}" && "${after}" == "${before}" ]]; then
      printf 'Fresh after-snapshot matched before-snapshot exactly.\n' >&2
    else
      printf 'Fresh after-snapshot did not prove exact before/after equality.\n' >&2
    fi
    die "ODOT canonical rehearsal failed; do not retry"
  fi
  if [[ -z "${after}" || "${after}" != "${before}" ]]; then
    printf 'Fresh one-shot server status after successful transaction snapshot failure (NO RETRY):\n' >&2
    run_psql --tuples-only --no-align --quiet --command="${status_sql}" >&2 || true
    die "rehearsal returned success but fresh exact before/after equality was not proven"
  fi
  rm -f -- "${sql_file}"
  trap - EXIT
  printf 'PASS: exact-one ODOT canonical hotfix compiled and verified; explicit ROLLBACK completed; fresh after-snapshot matched before-snapshot exactly.\n'
}
