...
const expectedInstalledFunctionMd5="2c4019a442101d2f03d38d90d3ad2b7f";
const expectedInstalledBaseFunctionMd5="7e1f03684f81ca07b5ad1bd1353aa178";
...
if (!/^[0-9a-f]{32}$/u.test(expectedInstalledFunctionMd5)
  || !/^[0-9a-f]{32}$/u.test(expectedInstalledBaseFunctionMd5)) {
  throw new Error("Permanent install is locked until passed rehearsal digests are pinned");
}
const release=loadRelease(repositoryRoot);
const startedAt=Date.now();
let phase="connect";
let transactionOpen=false;
let client=createClient("issue97-verified-official-pad-reference-permanent-install");
...
try {
  ...
  console.log(JSON.stringify({kind:"before",state:before}));
  ...
  if (inside.referenceFunction?.definitionMd5!==expectedInstalledFunctionMd5
    || inside.verifiedOfficialPadProof?.helper?.definitionMd5
      !==expectedInstalledBaseFunctionMd5
    || Number(inside.verifiedOfficialPadProof?.targetMigrationLedgerCount)!==1
    || inside.verifiedOfficialPadProof?.targetMigrationStatementMd5
  ...
  console.log(JSON.stringify({
    ...
    baseFunctionDefinitionMd5:after.verifiedOfficialPadProof.helper.definitionMd5,
  ...
  client=createClient("issue97-verified-official-pad-reference-failure-inspection");
  try {
    await client.connect();
    ...
    console.error(JSON.stringify({
      ...
      committed:Boolean(
        ...
        && Number(persisted.verifiedOfficialPadProof?.targetMigrationLedgerCount)===1
      ),
      state:persisted,
    }));
