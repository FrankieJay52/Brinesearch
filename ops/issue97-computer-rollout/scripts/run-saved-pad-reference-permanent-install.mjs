...
const expectedInstalledFunctionMd5 = "dc38da188bf4c4343dcf5cae5f1c41a7";
...
if (!/^[0-9a-f]{32}$/u.test(expectedInstalledFunctionMd5)) {
  throw new Error("Permanent install is locked until the passed rehearsal function digest is pinned");
}
const release = loadRelease(repositoryRoot);
const startedAt = Date.now();
let phase = "connect";
let transactionOpen = false;
let client = createClient("issue97-saved-pad-reference-permanent-install");
...
try {
  ...
  if (inside.referenceFunction?.definitionMd5 !== expectedInstalledFunctionMd5
    || Number(inside.migrationLedgerCount) !== 1
    || inside.migrationStatementMd5 !== release.migrationStatementMd5) {
  ...
  try {
    ...
    console.error(JSON.stringify({
      ...
      committed: Boolean(
        ...
        && Number(persisted.migrationLedgerCount) === 1,
      ),
      state: persisted,
    }));
