...
let client=createClient("issue97-verified-official-pad-reference-rehearsal");
...
try {
  ...
  if (!inside.referenceFunction
    ...
    || !inside.verifiedOfficialPadProof?.helper
    || Number(inside.verifiedOfficialPadProof?.targetMigrationLedgerCount)!==0) {
    throw new Error("Transaction-local function state violated rehearsal boundaries");
  }
  ...
  console.log(JSON.stringify({
    ...
    rehearsalBaseFunctionDefinitionMd5:
