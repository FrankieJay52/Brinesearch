...
const basemap = read("v18/src/features/owner-roads/ownerRoadBasemap.ts");
...
requireText(map, 'aria-label={basemapMode === "satellite" ? "Switch to road map" : "Switch to satellite imagery"}', "accessible road/satellite basemap toggle");
requireText(map, "ownerRoadBasemapStyle(basemapMode)", "display-only basemap style switch");
requireText(basemap, "USGSImageryOnly/MapServer/tile/{z}/{y}/{x}", "USGS public imagery tile service");
requireText(basemap, "U.S. Geological Survey, National Geospatial Program", "USGS imagery attribution");
forbid(basemap, /(?:supabase|route_prep|graph_build|google_route)/i, "satellite basemap crosses into route, graph, Google, or database authority");
requireText(mapModel, 'return "No exact route match"', "normalized held-pad no-match reason");
requireText(mapModel, 'return "Route not reconciled"', "held route without current graph association reason");
