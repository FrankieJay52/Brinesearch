...
describe("Batch 0 six-county Ascent navigation ledger", () => {
  it("contains the exact 247 identities and honest state/source accounting", () => {
    ...
    ]))).toEqual({ "1": 1, "2": 8, "3": 210, reviewed_handoff_authority_held: 28 });
    ...
    expect(ledger.filter((row) => row.current_state === "reviewed_handoff_authority_held").map((row) => row.name).sort())
      .toEqual(["ALBATROSS", "ATHENA", "BAKOS", "BANNOCK", "BEETLE", "BILINOVICH", "BRAVO", "CASTON", "CIRCLE-OAKS", "CROWIE", "DUKE", "GIL", "GILCHER", "HOOP", "LAKE", "LAWSON", "MALDON", "PICKENS", "PORTERFIELD B", "PORTERFIELD GAS UNIT", "ROCK RIDGE", "RUTH", "SADLER", "SKULL FORK", "THOMAS", "TOWE", "TROYER", "WITHEY"]);
  ...
  it("gives all 210 remaining state-3 pads one exact GPS-only Navigate with no route authority", () => {
    const stateThree = ledger.filter((row) => row.current_state === "3");
    expect(stateThree).toHaveLength(210);
  ...
  it("resolves all twenty-eight exact-record reviewed handoffs through the real page action builder", () => {
    const reviewed = ledger.filter((row) => row.current_state === "reviewed_handoff_authority_held");
    expect(reviewed).toHaveLength(28);
