  ...
  return <section className={`map-page${fullscreen ? " map-fullscreen" : ""}${roadMode ? " map-road-mode" : ""}`} data-viewer-mode={viewerMode}>
    ...
    {locationChoices ? <aside className="map-cluster-chooser" role="dialog" aria-modal="false" aria-labelledby="map-cluster-title">
      ...
      <details className="map-route-status"><summary><strong>Route status</strong><span>View</span></summary><div className="map-route-status-content">
        ...
        {selectedAscentPadRoadDisplay?.gpsLeg && <div className="selected-pad-route-key"><i className="legend-line gps-tether"/><strong>{selectedAscentPadRoadDisplay.gpsLeg.label} · thin solid neutral · unapproved</strong></div>}
        ...
        {activeSelectedAscentPadApproachDisplay?.lines.some((line) => line.colorRole === "gps") && <div className="selected-pad-route-key"><i className="legend-line gps-tether"/><strong>{selected.padName} lease road · thin solid neutral · unapproved</strong></div>}
