...
export function MapPage() {
  ...
  const ascentPadRoadScopeDetail = visibleAscentPadRoadDisplays.length
    ? ` ${visibleAscentPadRoadDisplays.length} reviewed Ascent route${visibleAscentPadRoadDisplays.length === 1 ? "" : "s"} remain visible in this scope. Solid teal follows the reviewed named roads and each stored final connector is labeled for its own pad as a lease road.${visibleAscentPadApproachDisplays.length ? ` ${visibleAscentPadApproachDisplays.length} additional measured last-highway approaches are loaded; exact matched sections and pad-specific final lease connectors are solid teal, while other unresolved access stays solid neutral.` : ""}`
    : "";
  ...
  return <section className={`map-page${fullscreen ? " map-fullscreen" : ""}${roadMode ? " map-road-mode" : ""}`} data-viewer-mode={viewerMode}>
    ...
    {locationChoices ? <aside className="map-cluster-chooser" role="dialog" aria-modal="false" aria-labelledby="map-cluster-title">
      ...
      {selectedFieldDirectionDisplay && <div className="selected-pad-field-direction" role="note" aria-label="BANNOCK selected road directions">
        ...
        <p>Red is not a restriction or closure. The marker stays at the exact saved GPS. Any stored final connector to the pin is teal and named only for this pad as BANNOCK lease road; it is not a reusable public-road identity. The Google Navigate link and public-road authority are unchanged.</p>
      </div>}
      {selectedAscentPadRoadDisplay && <div className="selected-pad-field-direction" role="note" aria-label={`${selected.padName} reviewed Ascent route`}>
        ...
        {selectedAscentPadRoadDisplay.gpsLeg && <span><i className="legend-line selected"/><strong>{selected.padName} lease road · solid teal</strong><small>Stored final connector to the saved pin · pad-specific, not a public-road identity</small></span>}
        ...
        <p>The saved GPS and Google Navigate link are unchanged. The pad-specific lease label completes the display to the pin but does not create or approve a Road Manager public-road identity.</p>
      </div>}
      {activeSelectedAscentPadApproach && <div className="selected-pad-field-direction" role="note" aria-label={`${selected.padName} last-highway approach`}>
        ...
        {activeSelectedAscentPadApproach.status === "ROUTED_DISPLAY" ? <>
          ...
          {activeSelectedAscentPadApproach.gpsTether?.nontrivial && <div className="selected-pad-route-key"><i className="legend-line selected"/><strong>{selected.padName} lease road · {activeSelectedAscentPadApproach.gpsTether.distanceMiles.toFixed(2)} mi · solid teal to saved pin</strong></div>}
          <p><strong>Measured public-road sections: {activeSelectedAscentPadApproach.mileage.roadDistanceMiles?.toFixed(2)} mi.</strong> {activeSelectedAscentPadApproach.gpsTether?.nontrivial ? `The separate ${selected.padName} lease road connector is excluded from that public-road total.` : "Road-section total only."}</p>
          <p>Exact reviewed identities and the pad-specific final lease connector are solid teal. Other unresolved sections stay solid neutral. A pad lease label cannot be reused as a public-road identity.</p>
      ...
      <details className="map-route-status"><summary><strong>Route status</strong><span>View</span></summary><div className="map-route-status-content">
        ...
        {selectedAscentPadRoadDisplay?.gpsLeg && <div className="selected-pad-route-key"><i className="legend-line selected"/><strong>{selected.padName} lease road · bright solid teal to saved pin</strong></div>}
        ...
        {activeSelectedAscentPadApproachDisplay?.lines.some((line) => line.lineRole === "pad_lease_road") && <div className="selected-pad-route-key"><i className="legend-line selected"/><strong>{selected.padName} lease road · bright solid teal to saved pin</strong></div>}
        ...
        {selectedFieldDirectionDisplay
          ? <p className="selection-route-note is-ready">BANNOCK selected display: solid teal arrives from OH-331 and continues over BANNOCK lease road to the saved pin; red exits by Black Oak Road to OH-149. The lease name is pad-specific, never a reusable public-road identity.</p>
          : selectedAscentPadRoadDisplay
            ? <p className="selection-route-note is-ready">{selected.padName} is highlighted through its reviewed named roads in solid teal.{selectedAscentPadRoadDisplay.gpsLeg ? ` The final connector is ${selected.padName} lease road and remains teal to the saved pin.` : " The reviewed line reaches the saved GPS without a separate lease connector."} Display creates no reusable public-road identity or release authority.</p>
          : activeSelectedAscentPadApproach
