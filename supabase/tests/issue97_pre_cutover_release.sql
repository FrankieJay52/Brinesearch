...
declare
  ...
  v_child jsonb;
begin
  if public.brinesearch_issue97_cutover_active() then raise exception '#97 pre-cutover suite requires cutover OFF'; end if;
  if exists(select 1 from public.brinesearch_road_graph_builds where status='staging') then raise exception '#97 pre-cutover suite refuses staging graphs'; end if;

  with expected(state_code,county_code) as (values
    ('OH','ATH'),('OH','BEL'),('OH','CAR'),('OH','COL'),('OH','COS'),('OH','GUE'),('OH','HAS'),('OH','HOL'),('OH','JEF'),('OH','LIC'),('OH','MAH'),('OH','MOE'),('OH','MUS'),('OH','NOB'),('OH','POR'),('OH','STA'),('OH','TRU'),('OH','TUS'),('OH','WAS'),
    ('WV','BRO'),('WV','DOD'),('WV','HAR'),('WV','MAR'),('WV','MSH'),('WV','MON'),('WV','OHI'),('WV','RIT'),('WV','TYL'),('WV','WET'),
    ('PA','ALL'),('PA','ARM'),('PA','BEA'),('PA','BRA'),('PA','BUT'),('PA','FAY'),('PA','GRE'),('PA','IND'),('PA','WAS'),('PA','WES')
  ), actual as (
  ...
  where e.state_code is null or a.state_code is null or a.active_count<>1 or a.current_count<>1;
  if v_count<>0 then raise exception '#97 exact 39-county active release-current manifest is incomplete'; end if;
  ...
  if v_count<>114 or exists(select 1 from required r where not private_verification.brinesearch_issue97_dataset_scope_current(r.dataset_id,r.state_code,r.county_code)) then
  ...
  if pg_catalog.has_function_privilege('service_role','public.brinesearch_issue97_activate_cutover_without_google_routes(jsonb)','EXECUTE')
     or pg_catalog.has_function_privilege('service_role','public.brinesearch_publish_structured_route_issue97_without_google(uuid,uuid,jsonb,bigint)','EXECUTE') then
    raise exception '#97 internal cutover/publisher bypass remains executable';
  end if;

  -- Public Google projection must still be dark before cutover.
