...
begin
  ...
  if p_patch ? 'pad_name' and pg_catalog.nullif(pg_catalog.btrim(p_patch->>'pad_name'), '') is null then
    raise exception 'pad_name_required' using errcode = '22023';
  end if;
  if p_patch ? 'company' and pg_catalog.nullif(pg_catalog.btrim(p_patch->>'company'), '') is null then
    raise exception 'company_required' using errcode = '22023';
  end if;
  if p_patch ? 'state' and pg_catalog.nullif(pg_catalog.btrim(p_patch->>'state'), '') is null then
  ...
  set
    ...
    county = case when p_patch ? 'county'
      then pg_catalog.nullif(pg_catalog.btrim(coalesce(p_patch->>'county','')), '') else p.county end,
    township = case when p_patch ? 'township'
      then pg_catalog.nullif(pg_catalog.btrim(coalesce(p_patch->>'township','')), '') else p.township end,
    address = case when p_patch ? 'address'
      then pg_catalog.nullif(pg_catalog.btrim(coalesce(p_patch->>'address','')), '') else p.address end,
    ...
    well_name = case when p_patch ? 'well_name'
      then pg_catalog.nullif(pg_catalog.btrim(coalesce(p_patch->>'well_name','')), '') else p.well_name end,
    api = case when p_patch ? 'api'
      then pg_catalog.nullif(pg_catalog.btrim(coalesce(p_patch->>'api','')), '') else p.api end,
    property_number = case when p_patch ? 'property_number'
      then pg_catalog.nullif(pg_catalog.btrim(coalesce(p_patch->>'property_number','')), '') else p.property_number end,
    ...
    structured_road_sequence = case when p_patch ? 'structured_road_sequence'
      then pg_catalog.nullif(pg_catalog.btrim(coalesce(p_patch->>'structured_road_sequence','')), '') else p.structured_road_sequence end,
    written_directions = case when p_patch ? 'written_directions'
      then pg_catalog.nullif(pg_catalog.btrim(coalesce(p_patch->>'written_directions','')), '') else p.written_directions end,
    directions_clear = case when p_patch ? 'directions_clear'
      then pg_catalog.nullif(pg_catalog.btrim(coalesce(p_patch->>'directions_clear','')), '') else p.directions_clear end,
    directions_clear_method = case when p_patch ? 'directions_clear'
      then case
        when pg_catalog.nullif(pg_catalog.btrim(coalesce(p_patch->>'directions_clear','')), '') is null then null
