...
-- Preserve the stricter fully-published implementation as a callable core.
...
begin
  ...
  if v_pad.latitude is null or v_pad.longitude is null
     ...
     or v_pad.longitude not between -180 and 180 then return false; end if;
  ...
  if v_route_count<>1 then return false; end if;
  select * into v_route
  from public.brinesearch_route_prep
  where pad_id=p_pad_id and active and route_group='primary'
  limit 1;
  if v_route.readiness_status<>'ready_for_road_matching' then return false; end if;
  ...
  if v_occ_count<2 then return false; end if;
  if exists(
    select 1 from private_verification.brinesearch_route_occurrence_receipts_issue97 o
    ...
    join public.brinesearch_road_identity_mappings m
      on m.identity_id=o.identity_id and m.road_id=o.canonical_road_id and m.mapping_status='verified'
    where o.route_prep_id=v_route.id
      and (
        o.resolution_status<>'resolved' or o.identity_id is null or o.canonical_road_id is null
  ...
  if v_geom_count<>v_occ_count then return false; end if;
  if exists(
    select 1
    from private_verification.brinesearch_route_occurrence_geometry_receipts_issue97 g
    ...
    where g.route_prep_id=v_route.id and (
      ...
      or (g.occurrence_index=1 and (g.geometry_kind<>'origin_anchor' or g.end_coordinate is null))
  ...
  if exists(
    ...
    where a.route_prep_id=v_route.id
      and not extensions.st_dwithin(a.end_coordinate::extensions.geography,b.start_coordinate::extensions.geography,1)
  ) then return false; end if;
  if not exists(
    select 1 from private_verification.brinesearch_route_occurrence_geometry_receipts_issue97 g
    where g.route_prep_id=v_route.id and g.occurrence_index=v_occ_count
      and g.status='resolved' and g.end_coordinate is not null
  ...
  if v_transition_count<>v_occ_count-1 then return false; end if;
  if exists(
    select 1
    from private_verification.brinesearch_route_transition_receipts_issue97 t
    where t.route_prep_id=v_route.id and (
      t.status<>'resolved' or t.coordinate is null or t.junction_id is null or t.anchor_id is null
      or t.graph_build_id is null or t.anchor_digest is null or t.source_revision_digest is null
  ...
  if coalesce((v_safety->>'has_hold')::boolean,false) then return false; end if;
  if private_verification.brinesearch_issue97_route_data_blocked(p_pad_id) then return false; end if;
  ...
  select pg_catalog.md5(pg_catalog.concat_ws('|',
    ...
    coalesce((select pg_catalog.string_agg(o.receipt_digest,'|' order by o.occurrence_index)
      from private_verification.brinesearch_route_occurrence_receipts_issue97 o where o.route_prep_id=v_route.id),''),
    coalesce((select pg_catalog.string_agg(g.receipt_digest,'|' order by g.occurrence_index)
      from private_verification.brinesearch_route_occurrence_geometry_receipts_issue97 g where g.route_prep_id=v_route.id),''),
    coalesce((select pg_catalog.string_agg(t.receipt_digest,'|' order by t.boundary_index)
      from private_verification.brinesearch_route_transition_receipts_issue97 t where t.route_prep_id=v_route.id),'')
  )) into v_dependency;

  return v_receipt.dependency_digest=v_dependency
    and v_receipt.manifest_digest is not null
    and exists(
      ...
      where pub.pad_id=p_pad_id
        ...
        and (pub.manifest->>'route_ready')::boolean
    );
end
$$;
...
begin
  ...
  if v_pad.latitude is null or v_pad.longitude is null
     or v_pad.latitude not between -90 and 90
     or v_pad.longitude not between -180 and 180 then
    return private_verification.brinesearch_issue97_hold_google_route(
      p_pad_id,v_route_revision,'pad_gps_missing_or_invalid',
      pg_catalog.jsonb_build_object('manifest_mode','transition_geometry','no_guess',true)
    );
  end if;
  ...
  if v_route_count<>1 then
    return private_verification.brinesearch_issue97_hold_google_route(
      ...
      case when v_route_count=0 then 'active_primary_route_prep_missing' else 'multiple_active_primary_route_preps' end,
      pg_catalog.jsonb_build_object('manifest_mode','transition_geometry','active_primary_count',v_route_count,'no_guess',true)
    );
  end if;
  select * into v_route from public.brinesearch_route_prep
  where pad_id=p_pad_id and active and route_group='primary' limit 1;
  if v_route.readiness_status<>'ready_for_road_matching' then
    return private_verification.brinesearch_issue97_hold_google_route(
      p_pad_id,v_route_revision,'route_prep_not_ready_for_road_matching',
  ...
  if v_occ_count<2 or exists(
    ...
    return private_verification.brinesearch_issue97_hold_google_route(
      p_pad_id,v_route_revision,'authoritative_occurrence_identity_incomplete',
      pg_catalog.jsonb_build_object('manifest_mode','transition_geometry','route_prep_id',v_route.id,'occurrence_count',v_occ_count,'no_guess',true)
    );
  end if;
  ...
  from private_verification.brinesearch_route_occurrence_geometry_receipts_issue97 g
  where g.route_prep_id=v_route.id;
  if v_geom_count<>v_occ_count or exists(
    select 1 from private_verification.brinesearch_route_occurrence_geometry_receipts_issue97 g
    where g.route_prep_id=v_route.id and (
      g.status<>'resolved'
      or (g.occurrence_index=1 and (g.geometry_kind<>'origin_anchor' or g.end_coordinate is null))
      or (g.occurrence_index>1 and (g.geometry_kind<>'traveled' or g.step_geometry is null))
    )
  ) then
    return private_verification.brinesearch_issue97_hold_google_route(
      p_pad_id,v_route_revision,'exact_occurrence_geometry_incomplete',
      pg_catalog.jsonb_build_object('manifest_mode','transition_geometry','route_prep_id',v_route.id,'expected',v_occ_count,'observed',v_geom_count,'no_guess',true)
    );
  end if;
  ...
  if exists(
    ...
    return private_verification.brinesearch_issue97_hold_google_route(
      p_pad_id,v_route_revision,'occurrence_geometry_not_continuous',
      pg_catalog.jsonb_build_object('manifest_mode','transition_geometry','route_prep_id',v_route.id,'no_guess',true)
    );
  end if;

  v_final_point:=extensions.st_setsrid(extensions.st_makepoint(v_pad.longitude,v_pad.latitude),4326);
  if not exists(
    select 1
    from private_verification.brinesearch_route_occurrence_geometry_receipts_issue97 g
    where g.route_prep_id=v_route.id and g.occurrence_index=v_occ_count
      and g.status='resolved' and g.end_coordinate is not null
      and extensions.st_dwithin(g.end_coordinate::extensions.geography,v_final_point::extensions.geography,1)
  ) then
    return private_verification.brinesearch_issue97_hold_google_route(
      p_pad_id,v_route_revision,'final_exact_road_does_not_reach_saved_pad_gps',
      pg_catalog.jsonb_build_object('manifest_mode','transition_geometry','route_prep_id',v_route.id,'no_guess',true,
        'policy','pad GPS may close only the already-resolved final road identity within one meter')
    );
  end if;
  ...
  from private_verification.brinesearch_route_transition_receipts_issue97 t
  where t.route_prep_id=v_route.id;
  if v_transition_count<>v_occ_count-1 or exists(
    ...
    return private_verification.brinesearch_issue97_hold_google_route(
      p_pad_id,v_route_revision,'exact_transition_chain_incomplete',
      pg_catalog.jsonb_build_object('manifest_mode','transition_geometry','route_prep_id',v_route.id,
        'expected_transitions',v_occ_count-1,'observed_transitions',v_transition_count,'no_guess',true)
    );
  end if;

  v_safety:=private_verification.brinesearch_issue97_pad_safety_facts(p_pad_id);
  v_safety_digest:=pg_catalog.md5(coalesce(v_safety,'{}'::jsonb)::text);
  if coalesce((v_safety->>'has_hold')::boolean,false)
     or private_verification.brinesearch_issue97_route_data_blocked(p_pad_id) then
    return private_verification.brinesearch_issue97_hold_google_route(
      p_pad_id,v_route_revision,'route_data_or_driver_safety_hold',
      pg_catalog.jsonb_build_object('manifest_mode','transition_geometry','route_prep_id',v_route.id,
        'safety_digest',v_safety_digest,'no_guess',true)
    );
  end if;

  -- Prove every traveled occurrence back to one current authoritative source
  -- identity/segment chain. Source names or proximity never choose the road;
  -- this is only provenance for the identity that reconciliation already fixed.
  ...
  with raw as (
    select
      ...
      d.source_key,d.topology_role,scope.last_success_run_id,
      ...
      extensions.st_unaryunion(extensions.st_collect(s.geom)) as source_chain,
      count(distinct s.id)::integer as source_segment_count,
      o.mapping_fingerprint,o.receipt_digest as occurrence_receipt_digest,
      g.receipt_digest as geometry_receipt_digest
    from private_verification.brinesearch_route_occurrence_geometry_receipts_issue97 g
    ...
    join public.brinesearch_authoritative_road_segments s
      on s.identity_id=i.id and s.active and s.dataset_id=i.dataset_id
    ...
    where g.route_prep_id=v_route.id and g.occurrence_index>1 and g.status='resolved'
      and d.topology_role in ('primary_network','supplemental_aliases')
    ...
    group by g.occurrence_index,g.route_prep_step_id,o.identity_id,o.canonical_road_id,
      i.dataset_id,i.state_code,i.county_code,i.source_identity_key,d.source_key,d.topology_role,
      scope.last_success_run_id,run.page_set_digest,g.step_geometry,
      o.mapping_fingerprint,o.receipt_digest,g.receipt_digest
  )
  select * from raw
  where source_segment_count>0
    and extensions.st_numgeometries(extensions.st_collectionextract(source_chain,2))=1
    and extensions.st_dwithin(source_chain::extensions.geography,step_geometry::extensions.geography,1);
  ...
  select l.*,
    (coalesce(sum(length_m) over(order by occurrence_index rows between unbounded preceding and current row),0)-length_m)::numeric as start_m,
    coalesce(sum(length_m) over(order by occurrence_index rows between unbounded preceding and current row),0)::numeric as end_m
  from lengths l;
  ...
  -- The first control point is the first exact transition. It is represented as
  -- a source-backed shape/route-ingress point so Google may use current location
  -- as origin but must enter the verified route here.
  insert into tmp_issue97_transition_google_points(sort_m,tie_order,point)
  select 0,10,pg_catalog.jsonb_build_object(
    'kind','shape','shape_role','route_ingress',
    'latitude',extensions.st_y(snapped)::numeric,
    'longitude',extensions.st_x(snapped)::numeric,
    ...
    'route_position_m',0,
    'distance_to_source_m',extensions.st_distance(snapped::extensions.geography,p.start_coordinate::extensions.geography)
  )
  from tmp_issue97_transition_google_path p
  join tmp_issue97_transition_google_sources src using(occurrence_index)
  cross join lateral (
    select extensions.st_closestpoint(src.source_chain,p.start_coordinate)::extensions.geometry(Point,4326) as snapped
  ) q
  where p.occurrence_index=2
    and extensions.st_dwithin(snapped::extensions.geography,p.start_coordinate::extensions.geography,1);
  ...
  -- Add route-aware curvature / regular-distance shaping candidates inside each
  -- exact traveled occurrence, then snap them back to the same current source
  -- chain. A candidate farther than one meter from source is discarded.
  insert into tmp_issue97_transition_google_points(sort_m,tie_order,point)
  with recursive
  vertices as (
    select p.occurrence_index,p.route_prep_step_id,p.start_m,p.length_m,p.step_geometry,
      (dp).path[1]::integer as vertex_index,(dp).geom::extensions.geometry(Point,4326) as geom,
      extensions.st_npoints(p.step_geometry) as vertex_count
    from tmp_issue97_transition_google_path p
    cross join lateral extensions.st_dumppoints(p.step_geometry) dp
  ), curvature as (
    select v.*,
      case when vertex_index<=1 or vertex_index>=vertex_count then 0::numeric else
        abs(degrees(atan2(
          sin(radians(extensions.st_azimuth(extensions.st_pointn(step_geometry,vertex_index-1)::extensions.geography,geom::extensions.geography))-
              radians(extensions.st_azimuth(geom::extensions.geography,extensions.st_pointn(step_geometry,vertex_index+1)::extensions.geography))),
          cos(radians(extensions.st_azimuth(extensions.st_pointn(step_geometry,vertex_index-1)::extensions.geography,geom::extensions.geography))-
              radians(extensions.st_azimuth(geom::extensions.geography,extensions.st_pointn(step_geometry,vertex_index+1)::extensions.geography)))
        )))::numeric end as turn_degrees,
      (start_m+extensions.st_length(extensions.st_linesubstring(step_geometry,0,
        extensions.st_linelocatepoint(step_geometry,geom))::extensions.geography))::numeric as route_position_m
    from vertices v
  ), regular as (
    select p.occurrence_index,p.route_prep_step_id,
      (p.start_m+gs.m)::numeric as route_position_m,
      extensions.st_lineinterpolatepoint(p.step_geometry,gs.m/nullif(p.length_m,0))::extensions.geometry(Point,4326) as geom,
      0::numeric as turn_degrees
    from tmp_issue97_transition_google_path p
    ...
    select occurrence_index,route_prep_step_id,route_position_m,geom,turn_degrees
    from curvature where vertex_index>1 and vertex_index<vertex_count and turn_degrees>=20
    union all
    select occurrence_index,route_prep_step_id,route_position_m,geom,turn_degrees from regular
  ), snapped as (
    select c.*,src.source_segment_keys,src.source_digest,src.last_success_run_id,src.source_page_set_digest,
      extensions.st_closestpoint(src.source_chain,c.geom)::extensions.geometry(Point,4326) as snapped_geom,
      extensions.st_distance(
        extensions.st_closestpoint(src.source_chain,c.geom)::extensions.geography,
        c.geom::extensions.geography
      ) as source_distance_m
    from candidates c join tmp_issue97_transition_google_sources src using(occurrence_index)
  ), ranked as (
    select s.*,row_number() over(partition by occurrence_index order by route_position_m) as rn,
      lag(route_position_m) over(partition by occurrence_index order by route_position_m) as previous_m
    from snapped s where source_distance_m<=1
  )
  select route_position_m,20,pg_catalog.jsonb_build_object(
    'kind','shape','shape_role','intermediate',
    'latitude',extensions.st_y(snapped_geom)::numeric,
    'longitude',extensions.st_x(snapped_geom)::numeric,
    'occurrence_id',route_prep_step_id,
    'source_kind','authoritative_clipped_geometry',
    'source_segment_keys',pg_catalog.to_jsonb(source_segment_keys),
    'source_digest',source_digest,
    'source_run_id',last_success_run_id,
    'source_page_set_digest',source_page_set_digest,
    'route_position_m',route_position_m,
    'distance_to_source_m',source_distance_m,
    'curvature_degrees',turn_degrees
  )
  from ranked
  where (previous_m is null or route_position_m-previous_m>=120) and rn<=12;

  -- Exact transition points after the ingress. Shared sections preserve their
  -- entry/exit semantics and shared geometry evidence instead of collapsing to
  -- a generic point junction.
  insert into tmp_issue97_transition_google_points(sort_m,tie_order,point)
  select p.end_m,30,pg_catalog.jsonb_strip_nulls(pg_catalog.jsonb_build_object(
    ...
    'shared_geometry_digest',case when j.shared_geometry is not null then pg_catalog.md5(extensions.st_asewkb(j.shared_geometry)) else null end,
    'shared_span_start',case when j.shared_geometry is not null then pg_catalog.jsonb_build_array(extensions.st_x(extensions.st_startpoint(j.shared_geometry)),extensions.st_y(extensions.st_startpoint(j.shared_geometry))) else null end,
    'shared_span_end',case when j.shared_geometry is not null then pg_catalog.jsonb_build_array(extensions.st_x(extensions.st_endpoint(j.shared_geometry)),extensions.st_y(extensions.st_endpoint(j.shared_geometry))) else null end,
  ...
  insert into tmp_issue97_transition_google_points(sort_m,tie_order,point)
  values(v_total_m+1,100,pg_catalog.jsonb_build_object(
    'kind','pad_destination','latitude',v_pad.latitude,'longitude',v_pad.longitude,
    'pad_id',p_pad_id,'source_kind','saved_pad_gps','route_position_m',v_total_m
  ));

  -- Drop rounded consecutive duplicates (for example an intermediate shape that
  -- lands exactly on the following junction) while always retaining ingress and
  -- the saved pad destination.
  with ordered as (
    select *,lag(pg_catalog.round((point->>'latitude')::numeric,7)::text||','||
  ...
  select pg_catalog.jsonb_agg(point order by sort_m,tie_order),count(*)::integer
  into v_points,v_point_count
  from ordered
  where previous_key is distinct from point_key or (point->>'kind') in ('shape','pad_destination') and (point->>'shape_role')='route_ingress';
  ...
  select pg_catalog.jsonb_agg(pg_catalog.jsonb_strip_nulls(pg_catalog.jsonb_build_object(
    ...
    'distance_miles',case when g.miles is not null then pg_catalog.round(g.miles::numeric,3) else null end
  ...
  select pg_catalog.md5(pg_catalog.concat_ws('|',
    v_route.id::text,coalesce(v_route.source_sequence_hash,''),coalesce(v_route.source_revision::text,''),
    pg_catalog.round(v_pad.latitude::numeric,7)::text,pg_catalog.round(v_pad.longitude::numeric,7)::text,
    coalesce((select pg_catalog.string_agg(o.receipt_digest,'|' order by o.occurrence_index)
      from private_verification.brinesearch_route_occurrence_receipts_issue97 o where o.route_prep_id=v_route.id),''),
    coalesce((select pg_catalog.string_agg(g.receipt_digest,'|' order by g.occurrence_index)
      from private_verification.brinesearch_route_occurrence_geometry_receipts_issue97 g where g.route_prep_id=v_route.id),''),
    coalesce((select pg_catalog.string_agg(t.receipt_digest,'|' order by t.boundary_index)
      from private_verification.brinesearch_route_transition_receipts_issue97 t where t.route_prep_id=v_route.id),''),
    coalesce((select pg_catalog.string_agg(src.source_digest||':'||src.source_page_set_digest,'|' order by src.occurrence_index)
  ...
  v_manifest:=pg_catalog.jsonb_build_object(
    ...
    'provenance',pg_catalog.jsonb_build_object(
      'route_occurrences','private verification exact authoritative identity receipts',
      'transitions','verified current #97 junction/anchor receipts',
      'geometry','exact canonical Road Manager occurrence geometry clipped between transition anchors',
      'shaping','current authoritative source segment chains within one meter',
      'destination','saved pad GPS on already-resolved final canonical road',
  ...
  insert into private_verification.brinesearch_google_route_receipts_issue97(
    ...
    pg_catalog.md5(coalesce((select pg_catalog.string_agg(src.source_digest,'|' order by src.occurrence_index) from tmp_issue97_transition_google_sources src),'')),
    pg_catalog.md5(coalesce((select pg_catalog.string_agg(t.graph_build_digest||':'||t.anchor_digest,'|' order by t.boundary_index)
      from private_verification.brinesearch_route_transition_receipts_issue97 t where t.route_prep_id=v_route.id),'')),
    pg_catalog.md5(coalesce((select pg_catalog.string_agg(o.mapping_fingerprint,'|' order by o.occurrence_index)
      from private_verification.brinesearch_route_occurrence_receipts_issue97 o where o.route_prep_id=v_route.id),'')),
    v_safety_digest,
    pg_catalog.jsonb_build_object('manifest_mode','transition_geometry','route_prep_id',v_route.id,
      'point_count',v_point_count,'occurrence_count',v_occ_count,'known_route_miles',pg_catalog.round(v_total_m/1609.344,3),
  ...
  on conflict(pad_id) do update set
    ...
    manifest_digest=excluded.manifest_digest,dependency_digest=excluded.dependency_digest,
    source_revision_digest=excluded.source_revision_digest,graph_dependency_digest=excluded.graph_dependency_digest,
    mapping_dependency_digest=excluded.mapping_dependency_digest,safety_digest=excluded.safety_digest,
    evidence=excluded.evidence,verified_at=excluded.verified_at,updated_at=excluded.updated_at;

  insert into public.brinesearch_driver_google_routes_public(pad_id,legacy_id,route_revision,source_revision,manifest,verified_at,updated_at)
  values(p_pad_id,v_pad.legacy_id,v_route_revision,coalesce(v_route.source_revision,0),v_manifest,v_now,v_now)
  on conflict(pad_id) do update set
    legacy_id=excluded.legacy_id,route_revision=excluded.route_revision,source_revision=excluded.source_revision,
    manifest=excluded.manifest,verified_at=excluded.verified_at,updated_at=excluded.updated_at;
  ...
  where id=p_pad_id;
  delete from private_verification.brinesearch_google_route_refresh_queue_issue97 where pad_id=p_pad_id;
...
declare
  v_mode text;
begin
  select coalesce(r.evidence->>'manifest_mode','published_route') into v_mode
...
grant execute on function public.brinesearch_issue97_google_route_current(uuid) to anon,authenticated,service_role;
...
comment on function private_verification.brinesearch_issue97_refresh_google_route_transition(uuid) is
  'Issue #97 exact-transition navigation manifest builder. Uses current authoritative identity/source/graph receipts and occurrence geometry; never name/nearest road selection. Full #69 publication remains separate.';
