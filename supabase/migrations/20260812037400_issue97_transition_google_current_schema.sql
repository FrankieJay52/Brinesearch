-- GitHub #97 — transition-geometry Google publisher aligned to the current receipt schema.
-- Service-only. Exact route occurrences, current source geometry and verified graph anchors
-- are required. Names/fuzzy/nearest-road evidence never select a control point.
...
begin
  ...
  with source_candidates as (
    select
      ...
      g.step_geometry,
      s.id as segment_id,s.source_segment_key,s.source_record_id,s.source_digest,s.geom
...
-- Static install-time guard against the obsolete runtime references found by the read-only audit.
...
begin
  if v_dep ilike '%last_success_run_id%'
     ...
     or v_refresh ilike '%j.junction_key%'
     or v_refresh ilike '%j.shared_geometry%'
