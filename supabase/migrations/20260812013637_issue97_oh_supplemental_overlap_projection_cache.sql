-- GitHub #97 — Ohio OGRIP overlap projection/cache hardening.
--
-- Carroll County contains 4,587 OGRIP centerlines and every row carries a name
-- event. Even after identity matching was indexed, recomputing ST_Transform and
-- ST_Buffer for each verified centerline/ODOT source-segment overlap exceeded the
-- production statement timeout. Cache projected centerlines/buffers and referenced
-- ODOT segments once per run, ANALYZE those temp sets, then perform the exact same
-- 0.25 m overlap intersection and fractional LRS measure derivation.
...
begin
  ...
  where r.id=p_run_id and r.status='loading' and r.state_code='OH'
    and d.source_key='oh_ogrip_lbrs_centerlines'
    and d.topology_role='supplemental_aliases'
  for update of r;
  if not found then raise exception 'Loading Ohio OGRIP supplemental ingest run not found' using errcode='P0002'; end if;
  ...
  with latest as (
    ...
    from public.brinesearch_road_source_ingest_runs r
    where r.dataset_id=v_odot_dataset and r.state_code='OH' and r.county_code=v_run.county_code
    order by r.started_at desc,r.id desc limit 1
  )
  select coalesce(pg_catalog.jsonb_agg(pg_catalog.jsonb_build_object(
    'dataset_id',v_odot_dataset,'source_key','oh_odot_tims_road_inventory',
    'state_code','OH','county_code',v_run.county_code,'run_id',run_id,
    'page_set_digest',page_set_digest,'completed_at',completed_at,
    'source_row_count',source_row_count,'ingested_row_count',ingested_row_count
  )),'[]'::jsonb) into v_target_runs
  from latest where private_verification.brinesearch_issue97_ingest_run_verified(run_id);
  if pg_catalog.jsonb_array_length(v_target_runs)<>1 then
    raise exception 'Complete current ODOT run is required before Ohio OGRIP materialization' using errcode='55000';
  end if;

  update public.brinesearch_authoritative_supplemental_centerlines
  set active=false
  where dataset_id=v_run.dataset_id and state_code='OH' and county_code=v_run.county_code
    and active and last_ingest_run_id is distinct from p_run_id;
  ...
  where m.active and exists(select 1 from public.brinesearch_authoritative_supplemental_centerlines c
    where c.id=m.centerline_id and c.dataset_id=v_run.dataset_id and c.state_code='OH'
      and c.county_code=v_run.county_code);
  update public.brinesearch_supplemental_centerline_dispositions d
  set active=false,updated_at=now()
  where d.dataset_id=v_run.dataset_id and d.state_code='OH' and d.county_code=v_run.county_code and d.active;
  ...
  with candidate_segments as (
    select c.id as centerline_id,a.identity_id,c.geom as source_geom,
      'OH:ODOT:SEGMENT:'||o.roadway_inventory_id as source_segment_key,o.geom as target_segment_geom
    ...
    join public.brinesearch_authoritative_segment_identity_assignments a
     ...
     and a.source_segment_key='OH:ODOT:SEGMENT:'||o.roadway_inventory_id and a.active
  ...
  create unique index tmp_issue97_oh_supp_centerline_proj_id_idx
    on tmp_issue97_oh_supp_centerline_proj(centerline_id);
  ...
  cross join lateral (
    select overlap_line.geom,
      extensions.st_linelocatepoint(o.geom_3857,extensions.st_startpoint(overlap_line.geom)) as start_fraction,
      extensions.st_linelocatepoint(o.geom_3857,extensions.st_endpoint(overlap_line.geom)) as end_fraction
    from (
      select (d).geom
      from extensions.st_dump(extensions.st_collectionextract(
        extensions.st_intersection(o.geom_3857,c.buffer_3857),2
      )) d
      order by extensions.st_length((d).geom) desc limit 1
    ) overlap_line
    where extensions.geometrytype(extensions.st_linemerge(o.geom_3857))='LINESTRING'
  ) overlap;
  ...
  select o.identity_id,v_run.dataset_id,
    ...
    case when o.ctl_begin is not null and o.ctl_end is not null then
      o.ctl_begin+(o.ctl_end-o.ctl_begin)*least(o.start_fraction,o.end_fraction) else o.ctl_begin end,
    case when o.ctl_begin is not null and o.ctl_end is not null then
      o.ctl_begin+(o.ctl_end-o.ctl_begin)*greatest(o.start_fraction,o.end_fraction) else o.ctl_end end,
    pg_catalog.jsonb_build_object(
      'supplemental_source_feature_key',o.source_feature_key,
      'supplemental_source_record_id',o.source_record_id,
      'supplemental_source_digest',o.source_digest,'source_name_event',event,
      'target_overlap',o.overlap_geojson,
      ...
      'projected_centerline_cached',true,'projected_odot_segment_cached',true,
...
comment on function public.brinesearch_issue97_refresh_supplemental_aliases_oh(uuid) is
  'Issue #97 Ohio OGRIP materializer. Exact ODOT identity mapping remains no-name/no-nearest; projected centerline buffers and referenced ODOT segments are cached once per run before exact overlap/fraction derivation.';
