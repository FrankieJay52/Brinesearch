-- BrineSearch V18 -- exact official map-reference coordinates for Ohio pads.
--
-- This public-safe contract fills a display gap only. It does not update a pad
-- coordinate, verify a driver entrance, create route steps or geometry, change
-- a graph, publish Google output, or alter cutover state. A reference is
-- admitted only from an exact, already-reviewed official pad attachment or an
-- exact official API/well attachment. Name-only and proximity matches are not
-- eligible.
...
begin
  ...
  into v_rows,v_row_count,v_pad_count,v_wellhead_count,v_content_sha256
  from reference_rows reference;

  if v_row_count>1000
     or v_row_count<>v_pad_count+v_wellhead_count
...
comment on function public.brinesearch_v18_pad_reference_coordinates(uuid) is
  'Display-only exact official pad/wellhead references for current Ohio directory pads lacking a verified entrance. Never route or Google authority.';
...
begin
  ...
  if v_payload is null
     ...
     or (v_payload->>'rowCount')::integer<>149
     ...
     or pg_catalog.jsonb_array_length(v_payload->'rows')<>149 then
