...
-- across all 19 Ohio counties. The prior graph builder detected shared sections
-- only from positive-length geometric intersection, which can miss an official
-- concurrency when ODOT's secondary line is slightly offset from its primary
-- line. No spatial tolerance, name, fuzzy match, or nearest-road guess is needed:
-- the source itself supplies the exact physical-pavement relationship.
--
...
-- Persistent source/authoritative geometry remains unchanged. Existing shared-
-- segment noding, multiway membership, anchors, names and provenance then run on
-- the source-declared physical pavement.
...
begin
  ...
  if pg_catalog.md5(v_definition)<>'793ed8985252b00d52f46da497484029'
    ...
    raise exception 'Issue #97 authoritative ODOT overlap builder did not install exactly';
  end if;
  ...
  if public.brinesearch_issue97_cutover_active() is distinct from v_before.cutover_active
    ...
    raise exception 'Issue #97 ODOT overlap migration changed graph or release state';
  end if;
end
