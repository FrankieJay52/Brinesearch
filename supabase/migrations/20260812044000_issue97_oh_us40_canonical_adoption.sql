-- GitHub #97 — Ohio-only exact US-40 canonical adoption.
--
-- CROWIE is the final currently resolved Ohio occurrence whose exact authoritative
-- identity lacks a canonical Road Manager mapping. Ohio US-40 has one existing
-- legacy family row and nine active/current ODOT identities across the current
-- Ohio source footprint. Upgrade that one family row and map only exact Ohio
-- US-40 identities. No name/fuzzy/nearest proof is permitted.

do $issue97_oh_us40_canonical_adoption$
declare
  v_road_id uuid;
  v_row_count integer;
...
begin
  ...
  where r.road_type='us_route' and r.route_number='40' and r.state is null;
  if v_row_count<>1 then
    raise exception 'Issue #97 Ohio US-40 adoption expected one Road Manager family row, found %',v_row_count;
  end if;
  ...
  where i.state_code='OH' and i.active
    and i.road_class='us_route' and i.route_number='40'
  ...
  if coalesce(v_identity_count,0)=0 or not coalesce(v_all_current,false)
    ...
    raise exception 'Issue #97 Ohio US-40 authoritative family is incomplete';
  end if;
  ...
  if v_conflict_count<>0 then
    raise exception 'Issue #97 Ohio US-40 adoption found % conflicting verified mappings',v_conflict_count;
  end if;
  ...
  from unnest(
    ...
    ||array['US-40','Route 40','U.S. 40']::text[]
  ) alias_value;

  update public.brinesearch_roads r set
    canonical_name='US-40',normalized_name='us-40',road_type='us_route',
    state=null,county=null,township=null,route_number='40',
    ...
    candidate_basis=coalesce(r.candidate_basis,'{}'::jsonb)||pg_catalog.jsonb_build_object(
      ...
      'route_family','US-40','source_identity_count',v_identity_count,
    ...
    coverage_states=array['OH']::text[],updated_at=now()
  ...
  select
    ...
    pg_catalog.jsonb_build_object(
      ...
      'road_class','us_route','route_number','40',
  ...
  if exists(
    ...
    where m.evidence->>'migration'='issue97_oh_us40_canonical_adoption'
      and (i.state_code<>'OH' or i.road_class<>'us_route' or i.route_number<>'40'
    ...
    raise exception 'Issue #97 Ohio US-40 adoption violated the Ohio-only/no-guess contract';
  end if;
end
$issue97_oh_us40_canonical_adoption$;

do $issue97_verify_oh_us40_canonical_adoption$
declare
...
begin
  ...
  from public.brinesearch_road_identity_mappings m
  join public.brinesearch_authoritative_road_identities i on i.id=m.identity_id
  where i.state_code='OH' and i.active and i.road_class='us_route' and i.route_number='40'
    and i.public_access_status='public' and i.drivable_status='drivable'
    and m.mapping_status='verified' and m.mapping_method='exact_route_designation'
    and m.road_id=(select id from public.brinesearch_roads where road_type='us_route' and route_number='40' and state is null);
  if v_family_rows<>1 or v_mapped<>9 then
    raise exception 'Issue #97 Ohio US-40 adoption verification failed: family %, mapped %',v_family_rows,v_mapped;
  end if;
end
