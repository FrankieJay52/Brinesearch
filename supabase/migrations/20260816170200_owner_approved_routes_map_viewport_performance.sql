-- Issue #108 performance and classification correction.
-- Keep routine viewports on indexed OH/external geometry stores, batch source
-- currentness checks, and require exact structured policy-route identity.

-- The previous two unpublished Issue #108 migrations used the original signature.
-- Replace that draft-only function instead of leaving a second callable overload.
drop function if exists public.owner_approved_routes_map_viewport(
  double precision,double precision,double precision,double precision,
  integer,text,text,text[],text[],text,uuid,integer
);
