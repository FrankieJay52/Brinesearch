-- GitHub #97 — Ohio supplemental OGRIP -> ODOT exact-equivalence performance hardening.
--
-- Production Harrison County rollout showed that source download/paging completed,
-- but supplemental alias finalization timed out while spatially joining OGRIP
-- centerlines through the security-barrier normalized road-segment view. That view
-- is correct for browser/routing reads, but it prevents the service-only batch job
-- from taking the indexed ODOT source-table path at statewide scale.
--
-- Keep the exact same no-name/no-nearest equivalence contract and 0.25 m geometry
-- tolerance, but execute Ohio materialization directly against:
...
-- with an indexed bounding-box prefilter. Pennsylvania continues through the
-- previously audited core implementation unchanged.

create index if not exists brinesearch_supp_centerline_scope_run_issue97_idx
on public.brinesearch_authoritative_supplemental_centerlines(
  dataset_id,county_code,last_ingest_run_id,active
);

create index if not exists brinesearch_supp_centerline_geom_issue97_idx
on public.brinesearch_authoritative_supplemental_centerlines
using gist(geom);

-- Preserve the previously audited implementation as the non-Ohio core.
do $issue97_rename$
begin
  if pg_catalog.to_regprocedure('public.brinesearch_issue97_refresh_supplemental_aliases_issue97_core(uuid)') is null then
    execute 'alter function public.brinesearch_issue97_refresh_supplemental_aliases(uuid) rename to brinesearch_issue97_refresh_supplemental_aliases_issue97_core';
  end if;
end
$issue97_rename$;
...
begin
  ...
  where n.source_dataset_id=v_run.dataset_id and n.active
    and exists(
      ...
      where i.id=n.identity_id and i.state_code='OH'
        and i.county_code=v_run.county_code
    );
  ...
  with candidate_segments as (
    ...
    join public.brinesearch_odot_road_catalog o
     ...
     and not extensions.st_isempty(o.geom)
     and extensions.st_isvalid(o.geom)
     and extensions.st_issimple(o.geom)
     and extensions.st_dimension(o.geom)=1
     and extensions.st_coveredby(
       o.geom,extensions.st_makeenvelope(-180,-90,180,90,4326)
     )
    ...
    select centerline_id,identity_id,
      ...
      extensions.st_unaryunion(
        extensions.st_collect(extensions.st_transform(target_segment_geom,3857))
      ) as target_geom
    from candidate_segments
    group by centerline_id,identity_id
  ), exact_matches as (
    select centerline_id,identity_id,source_segment_keys,
      greatest(
        extensions.st_distance(
          extensions.st_startpoint(extensions.st_linemerge(extensions.st_transform(source_geom,3857))),
          target_geom
        ),
        extensions.st_distance(
          extensions.st_endpoint(extensions.st_linemerge(extensions.st_transform(source_geom,3857))),
          target_geom
        )
    ...
    where extensions.geometrytype(
      extensions.st_linemerge(extensions.st_transform(source_geom,3857))
    )='LINESTRING'
  )
  select centerline_id,v_run.dataset_id,'OH',v_run.county_code,
    ...
    pg_catalog.jsonb_build_object(
      ...
      'nearest_road_used_for_mapping',false,'many_to_many_preserved',true,
      'containment_direction_preserved',true,
  ...
  select m.identity_id,v_run.dataset_id,
    ...
    segment_key,o.ctl_begin,o.ctl_end,
    pg_catalog.jsonb_build_object(
  ...
  return pg_catalog.jsonb_build_object(
    'run_id',p_run_id,'mappings',v_mapping_count,
    'name_events_materialized',v_name_count,'name_matching_used',false,
    'nearest_road_matching_used',false,'target_primary_runs',v_target_runs,
    'centerline_dispositions',v_disposition_count,
...
create or replace function public.brinesearch_issue97_refresh_supplemental_aliases(
  p_run_id uuid
)
returns jsonb
language plpgsql
...
begin
  ...
  from public.brinesearch_road_source_ingest_runs
  where id=p_run_id;
  if not found then
    raise exception 'Supplemental ingest run not found' using errcode='P0002';
  end if;
  if v_state='OH' then
...
grant execute on function public.brinesearch_issue97_refresh_supplemental_aliases(uuid)
to service_role;

comment on function public.brinesearch_issue97_refresh_supplemental_aliases_oh(uuid) is
  'Issue #97 Ohio OGRIP alias materializer. Uses indexed direct ODOT source geometry plus exact active segment assignments; names and nearest-road heuristics are excluded from identity mapping.';
comment on function public.brinesearch_issue97_refresh_supplemental_aliases(uuid) is
  'Issue #97 supplemental alias materializer dispatcher. Ohio uses the indexed direct ODOT source path; Pennsylvania preserves the prior audited implementation.';
