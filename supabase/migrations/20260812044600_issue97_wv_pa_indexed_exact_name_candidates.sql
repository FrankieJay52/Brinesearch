...
begin
  ...
  if pg_catalog.strpos(v_definition,v_old)=0 then
    raise exception 'Issue #97 WV/PA indexed exact-name patch target not found';
  end if;
  if pg_catalog.strpos(pg_catalog.substr(v_definition,pg_catalog.strpos(v_definition,v_old)+1),v_old)>0 then
    raise exception 'Issue #97 WV/PA indexed exact-name patch target is not unique';
  end if;
...
begin
  if exists(
    ...
    where i.state_code in ('WV','PA') and i.active and n.active
      and coalesce(n.normalized_name,'') is distinct from
          private_verification.brinesearch_issue97_normalize_route_token(n.road_name)
  ) then
    raise exception 'Issue #97 WV/PA normalized-name repair did not converge';
  end if;
  ...
  if v_definition not like '%where i.state_code=v_state and i.active and i.normalized_name=v_token%'
     or v_definition not like '%where n.active and n.normalized_name=v_token%'
     or v_definition not like '%join public.brinesearch_authoritative_road_identities i%on i.id=hit.identity_id and i.state_code=v_state and i.active%'
     or v_definition like '%similarity(%'
     or v_definition like '%nearest_road_used'',true%'
  then
    raise exception 'Issue #97 WV/PA indexed exact-name candidate contract did not install cleanly';
  end if;
end
