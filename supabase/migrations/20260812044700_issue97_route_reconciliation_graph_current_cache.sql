...
begin
  ...
  if pg_catalog.strpos(v_definition,v_old)=0 then
    raise exception 'Issue #97 route-corpus graph-cache patch target not found';
  end if;
  if pg_catalog.strpos(pg_catalog.substr(v_definition,pg_catalog.strpos(v_definition,v_old)+1),v_old)>0 then
    raise exception 'Issue #97 route-corpus graph-cache patch target is not unique';
  end if;
...
begin
  ...
  if v_definition not like '%perform private_verification.brinesearch_issue97_prepare_graph_current_cache();%'
     or v_definition not like '%brinesearch_issue97_resolve_route_identity_path(v_route.id)%'
  then
    raise exception 'Issue #97 route-corpus graph-current cache contract did not install cleanly';
  end if;
end
