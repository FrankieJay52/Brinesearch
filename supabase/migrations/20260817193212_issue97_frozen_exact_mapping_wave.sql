  ...
  if (select count(*) from (
      ...
      where route.active and (
        exists(select 1 from public.brinesearch_route_prep_steps step
          join (select distinct road_id from tmp_issue97_frozen_mapping_targets) target on target.road_id=step.road_id
        ...
        or exists(select 1 from public.brinesearch_route_prep_steps step
          ...
          where step.route_prep_id=route.id and step.active)
        or exists(select 1 from private_verification.brinesearch_route_occurrence_receipts_issue97 receipt
     ...
     or (select pg_catalog.md5(pg_catalog.string_agg(id::text,'|' order by id)) from (
      select distinct route.id
      from public.brinesearch_route_prep route
      where route.active and (
        exists(select 1 from public.brinesearch_route_prep_steps step
        ...
        or exists(select 1 from private_verification.brinesearch_route_occurrence_receipts_issue97 receipt
          join tmp_issue97_frozen_mapping_targets target on target.identity_id=receipt.identity_id
          where receipt.route_prep_id=route.id)
      )) closure)<>'836c1f57210e4d18c0f60d4c1ea77d7d'
  then
    raise exception 'Issue #97 frozen 379-route closure drifted';
  end if;
