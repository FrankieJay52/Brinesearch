...
-- Generic Route Prep refresh used to leave an old road_id/mileage attached when a
-- pending step changed text at the same index. Clear it before generic rematching.
...
-- Load and validate seven exact official Ohio LBRS road identities. Burrier is the
-- only current Road Manager row corrected in place; the other six are new identities.
do $source$
declare
  ...
  names text[]; digest text; road_id uuid; existing_count int; ref_count int;
  expected_jur text; expected_road_type text; alias_base text;
  source_url constant text:='https://maps.ohio.gov/arcgis/rest/services/Hosted/Ohio_Statewide_LBRS_Centerlines/FeatureServer/0';
  query_url text;
begin
  ...
  loop
    ...
    query_url:=source_url||'/query?where=NLFID%3D%27'||replace(rec.nlfid,'*','%2A')||
    ...
    if rec.correct_existing then
      select count(*),(min(r.id::text))::uuid into existing_count,road_id
      ...
      where s.active and s.road_id=road_id;
      if ref_count<>2 or exists(
        ...
        where s.active and s.road_id=road_id and (lower(btrim(s.raw_text))<>'burrier rd' or not p.legacy_id=any(rec.legacy_ids))
      ) then raise exception '#70 Burrier row has unexpected references'; end if;
      if exists(select 1 from public.brinesearch_roads r where r.id<>road_id and r.state='OH' and lower(coalesce(r.county,''))=lower(rec.county) and r.road_type='township' and r.route_number='135') then
      ...
      update public.brinesearch_roads set
        ...
        source_url=source_url,source_record_id=rec.nlfid||'|route:TR:135',centerline_geojson=extensions.st_asgeojson(geom,7)::jsonb,
      ...
      insert into public.brinesearch_roads(
        ...
        'State of Ohio','Ohio Statewide LBRS Centerlines','official_ohio_lbrs_issue70_source_conflict_correction',source_url,
    ...
    insert into private_verification.brinesearch_source_conflict_corrections_issue70(
      ...
      feature_count,names,digest,source_url,road_id,jsonb_build_object(
...
-- Exact expected old stored direction labels must still be present before mutation.
do $guard$
begin
...
-- Batch the five authoritative pad updates without invoking the expensive all-pad
-- direction-intelligence refresh five times; refresh once after all source changes.
...
-- Rebuild only the five corrected primary Route Prep records; JANIE's parser-created
-- alternate OR variant is deactivated. No global Owner refresh is invoked by migration.
...
-- Rewrite only durable evidence rows whose saved type/number is now disproven.
