...
$new$;
  v_old_dataset text:='set fetched_at=now(),content_digest=v_content_digest,';
  v_new_dataset text:=$dataset$
...
begin
  ...
  v_digest_count:=(pg_catalog.length(v_definition)-pg_catalog.length(pg_catalog.replace(v_definition,v_old_digest,'')))
    /pg_catalog.length(v_old_digest);
  v_dataset_count:=(pg_catalog.length(v_definition)-pg_catalog.length(pg_catalog.replace(v_definition,v_old_dataset,'')))
    /pg_catalog.length(v_old_dataset);
  if v_digest_count<>1 or v_dataset_count<>1 then
    raise exception 'Issue #97 scope digest patch changed unexpectedly: digest=% dataset=%',v_digest_count,v_dataset_count;
  end if;

  v_definition:=pg_catalog.replace(v_definition,v_old_digest,v_new_digest);
  v_definition:=pg_catalog.replace(v_definition,v_old_dataset,v_new_dataset);
