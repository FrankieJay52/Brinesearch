...
begin
  ...
  if v_start_pos<1 then raise exception 'Issue #97 optimized name-change region missing'; end if;
  v_end_rel := pg_catalog.strpos(pg_catalog.substr(v_definition,v_start_pos),'insert into public.brinesearch_road_junctions(');
  if v_end_rel<1 then raise exception 'Issue #97 name-change downstream boundary missing'; end if;
  ...
  then raise exception 'Issue #97 optimized name-change contract did not install cleanly'; end if;
end
