...
--
-- The inner set-based fingerprint uses the exact same fields and ordering as
-- brinesearch_issue97_mapping_fingerprint(identity_id). Source-run freshness and
-- required-scope checks are unchanged.
      ...
      from (
        select identities.identity_id,
          pg_catalog.md5(pg_catalog.concat_ws('|',
            ...
            coalesce(m.road_id::text,'unmapped'),
...
begin
  ...
  if v_definition like '%brinesearch_issue97_mapping_fingerprint(current_map.identity_id)%'
     or v_definition not like '%current_map.fingerprint%'
